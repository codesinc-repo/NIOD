...
const HeroSection = () => (
  <section className="relative overflow-hidden bg-white font-['Inter',sans-serif]">
    <div className="relative mx-auto max-w-7xl flex flex-col md:grid md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.4fr)] items-stretch">
      ...
      <div className="order-1 md:order-2 flex items-center justify-center w-full">
        <img
          ...
          className="w-full h-auto max-h-[320px] md:max-h-none object-contain object-center"
        />
      </div>

      {/* ---- Text content: below image on mobile, left column on md+ ---- */}
      <div className="order-2 md:order-1 flex flex-col justify-center px-5 py-8 md:pl-[6%] md:pr-0 md:py-16 lg:pl-[10%] lg:py-20">
        <div className="max-w-[440px] text-[#1b3a22]">
          <h1 className="text-[clamp(26px,3.6vw,44px)] font-extrabold leading-[1.05] tracking-[-0.025em] text-[#1b4423]">
            Pure by Nature,
            <br />
            Made with Care.
          </h1>
          ...
          <p className="mt-5 max-w-[340px] text-[15px] leading-[1.65] text-[#3a4a3f]">
          ...
          <ul className="mt-10 grid grid-cols-4 gap-3 text-center">
            {BADGES.map(({ Icon, label }) => (
              <li key={label} className="flex flex-col items-center gap-2">
                <span className="flex h-[46px] w-[46px] items-center justify-center rounded-full border border-[#1b4423]/70 text-[#1b4423]">
                  <Icon className="h-5 w-5" />
                </span>
                <span className="text-[11px] font-medium leading-tight text-[#1b3a22]">
                  {label}
                </span>
              </li>
            ))}
